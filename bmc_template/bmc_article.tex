...
\caption{\csentence{Illustration of \textit{Exchange (\textbf{M},\textbf{N})} method.}
             This method is applied to tree $g$ and proposes $g'$ by swapping \textbf{M} and \textbf{N}, so that the three distances are adjusted to maintain the distances among \textbf{S}, \textbf{M} and \textbf{N}. That is, $d_{N'} = d_C + d_N$ and $d_{M'} = d_{M} - d_{C'}$.}
