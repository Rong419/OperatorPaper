...
In contrast to Simple Distance, Small Pulley proposes new genetic distances of branches on both sides of the root. As is illustrated in Figure \ref{simpledistance}(d), two new rates in tree ${g}'$ is proposed based on those in the original tree $g$. In order to maintain the total genetic distance $d_L + d_R$ of the two branches linked to the root, after ${d_L}'$ is proposed, $d_R$ will be adjusted simultaneously. In other words, Small Pulley keeps $D = d_L + d_R$ constant. The detailed process includes the following 4 steps.
