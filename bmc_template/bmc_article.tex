...
\begin{algorithmic}
\STATE \COMMENT{Step 1: Identify the child node of the root \textbf{X} that has two child nodes below, which is denoted by \textbf{O}. The other child node of the root, which does not have any child nodes, is denoted by \textbf{Y}. The node times of the root \textbf{X},  \textbf{Y}, \textbf{O} and its child nodes are denoted by ${t_X}$, ${t_Y}$, ${t_O}$, ${t_{G1}}$ and ${t_{G2}}$ respectively.}
\STATE Let \textbf{X} be the root of the tree.
