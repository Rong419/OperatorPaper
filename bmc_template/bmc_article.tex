...
The results are shown in Figure \ref{correlation}. The tree in Figure \ref{Fig.sub.tree} is used to filter the sampled trees to ensure that all trees examined have the same topology. Afterwards, a pairwise comparison was made by using 12 branch rates and 6 node times. As can be seen from upper right of Figure \ref{Fig.sub.correlation}, to a large degree, the rates are negatively correlated with the node times. This indicates that when a larger node time is proposed, the corresponding rates become smaller. For example, $t1$ has a negative correlation with $r1$ and $r2$, but has positive correlation with $r8$. That is to say, with the increase of $t1$, the rate $r8$ will also increase, but $r1$ and $r2$ will decrease. In addition, in the upper left of Figure \ref{Fig.sub.correlation}, most rates on two adjacent branches have positive correlations, but have a negative correlation with the rate above them. Take $r5$ and $r6$ as an example. They have a positive correlation with each other, but are negatively related to $r11$. This means a larger $r5$ incurs a larger $r6$ and a smaller $r11$. To sum up, this dynamic change of rates and node times is consistent with the mechanism of the proposed operator. Although there are some inconsistent correlations, it should be noticed that this is an average pairwise comparison in two dimensions. For comparisons in higher dimensions, the results would be closer to the mechanism of the proposed operator.
...
The resulting summary trees are shown in Figure \ref{summary3}. As can be seen, the most probable tree in Figure  \ref{summary3} (b) is consistent with previous analyses of this data \cite{cooper2001complete} (Figure \ref{ratites}). For large data sets of long sequences, this method may prove useful to provide faster divergence time estimates based on the assumption of known unrooted topology and branch lengths in units of expected substitutions per site.
...
As data sets increase in size the impetus to improve efficiency of Bayesian phylogenetic inference algorithms has steadily increased. Besides more effective proposal mechanisms within Metropolis-Hastings MCMC, completely novel approaches to Bayesian phylogenetics have also begun to get some attention. Variational methods are a non-stochastic approach to approximating Bayesian inference \cite{beal2003variational}. These approaches make inference an optimisation problem and take advantages of a set of tractable distributions, which decreases the computational cost by avoiding high-dimensional integrals in MCMC sampling schemes. Recent work has investigated the potential for applying variational methods to phylogenetics \cite{zhang2018variational,dang2019stochastic}. Our improved MCMC methods provide a mature performance baseline for these new approaches.
...
%\label{Fig.sub.1}
%\includegraphics[width=0.45\textwidth]{scenario1S}}
%\subfigure[Senario 1: chain length = 20000000]{
%\label{Fig.sub.2}
%\includegraphics[width=0.45\textwidth]{scenario1L}}
%\subfigure[Senario 2: chain length = 10000000]{
%\label{Fig.sub.3}
%\includegraphics[width=0.45\textwidth]{scenario2S}}
%\subfigure[Senario 2: chain length = 20000000]{
%\label{Fig.sub.4}
%\includegraphics[width=0.45\textwidth]{scenario2L}}
...
%\label{Fig.sub.5}
%\includegraphics[width=0.45\textwidth]{RplotSD}}
%\subfigure[$d_D$ in Small Pulley]{
%\label{Fig.sub.6}
%\includegraphics[width=0.45\textwidth]{RplotSP}}
%\subfigure[${d_D}$ in Big pulley]{
%\label{Fig.sub.9}
%\includegraphics[width=0.45\textwidth]{bigd}}
%\subfigure[${t_E}$ in Big pulley]{
%\label{Fig.sub.10}
%\includegraphics[width=0.45\textwidth]{bigT}}
...
%\subfigure{
%\label{P_ess}
%\includegraphics[width=0.45\textwidth]{primatesESS.eps}}
%\subfigure{
%\label{P_run}
%\includegraphics[width=0.45\textwidth]{primatestime.eps}}
%\caption{\csentence{Comparison of ESS and running time using primates data.}
...
%\caption{\csentence{Correlation between rates and node times in the ratites tree.}
%             The rates and node times are in correspondence with the notations in Figure \ref{withoutdata}(b). Blue indicates the positive relation and red indicates the negative relation. The darker the colour is, the stronger the relation tends to be.}
%\label{correlation}
%\end{figure}
%
%\begin{figure}[h!]
...
%\label{Fig.sub.11}
%\includegraphics[width=0.45\textwidth]{unrootedtree}}
%\subfigure[Summary tree with 94.44\% probability]{
%\label{summary1}
%\includegraphics[width=0.45\textwidth]{First}}
%\subfigure[Summary tree with 4.24\% probability]{
%\label{summary2}
%\includegraphics[width=0.45\textwidth]{Second}}
%\subfigure[Summary tree with 1.32\% probability]{
%\label{summary3}
%\includegraphics[width=0.45\textwidth]{Third}}
%\caption{\csentence{Illustration of sampling a fixed unrooted tree.}
