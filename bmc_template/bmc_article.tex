...
\item Propose ${d_C}' \leftarrow {d_C} + b$, where $b \sim U[ - v, + v]$. Make sure that $0 < {d_C}' < D$ holds, where $D = {d_C} + {d_{N}}$. Otherwise, we reject the proposal.
...
\emph{Step 3} Propose a new node time either for \textbf{L} or \textbf{R}. And apply the method to the selected node and one of its sibling's children, as is detailed by Algorithm \ref{symmetric tree proposal}.
\begin{algorithm}
\caption{Proposal for symmetric trees in Big pulley}
\label{symmetric tree proposal}
\begin{algorithmic}
\STATE Draw a random number $\sigma_1$ from Unfirm[0,1].
\IF{${\sigma_1} < 0.5$}
\STATE Pick \textbf{L} and propose a new node time by ${t_L}' = {t_L} + {a_1}$, where ${a_1} \sim U[ - w, + w]$. 
\IF{${t_R} < {t_L}' < {t_X}'$}
\STATE Draw a random number $\sigma_2$ from Unfirm[0,1]. 
\IF{${\sigma_2} < 0.5$}
...
\ENDIF
\ELSE
\STATE Pick \textbf{R} and propose a new node time by ${t_R}' = {t_R} + {a_2}$, where ${a_2} \sim Uniform[ - w, + w]$. 
\IF{${t_L} < {t_R}' < {t_X}'$}
\STATE Draw a random number $\sigma_3$ from Unfirm[0,1]. 
\IF{${\sigma_3} < 0.5$}
...
%\begin{itemize}
%\item With 0.5 probability pick \textbf{L} and propose a new node time by ${t_L}' = {t_L} + {a_1}$, where ${a_1} \sim U[ - w, + w]$. Make sure that ${t_R} < {t_L}' < {t_X}'$ holds. Otherwise, we reject the proposal. If we don't reject then there are two options to apply the method:
%\begin{itemize}
%\item With probability 0.5 apply \textit{Exchange (\textbf{H1}, \textbf{R})} and propose tree \textcircled1
%
%\item otherwise apply \textit{Exchange (\textbf{H2}, \textbf{R})} and propose tree \textcircled2
%\end{itemize}
%
%\item Otherwise pick \textbf{R} and propose a new node time by ${t_R}' = {t_R} + {a_2}$, where ${a_2} \sim Uniform[ - w, + w]$. Make sure that ${t_L} < {t_R}' < {t_X}'$ holds. Otherwise, we reject the proposal. Similarly, if we don't reject then there are two options to apply the method:
%  \begin{itemize}
%  \item With probability 0.5 apply \textit{Exchange (\textbf{H3}, \textbf{L})} and propose tree \textcircled3
%  \item With probability 0.5 apply \textit{Exchange (\textbf{H4}, \textbf{L})} and propose tree \textcircled4
%  \end{itemize}
%\end{itemize}
...
\emph{Step 3} Apply the method using \textbf{Y} and either child node of \textbf{O}, which is described in Algorithm \ref{asymmetric tree proposal}.
\begin{algorithm}
\caption{Proposal for asymmetric trees in Big pulley}
\label{asymmetric tree proposal}
\begin{algorithmic}
\IF{${t_O}' > \max \{ {t_{G1}},{t_{G2}}\} $ or ${t_{G1}} = {t_{G2}}$}
\STATE Draw a random number $\sigma_4$ from Unfirm[0,1]. 
\IF{${\sigma_4} < 0.5$}
...
\end{figure}
% \alexei{This is not correct. It is not possible to specify the lognormal stdev in real space in BEAST 2}
