...
Likewise, the Jacobian matrix for Simple Distance, Small Pulley and Big Pulley can be obtained. For the strategy of Big Pulley, the operations are concerned with tree topology. To make the proposed topology reversible, a factor $\mu$ is defined and will be included in the Hastings ratio, which is calculated by Algorithm \ref{alg1}. 
%Therefore, the Hastings ratio when the proposed operator is working on the root of a phylogenetic tree is derived by Eq. 
%%Complete the r_root
\begin{algorithm}
