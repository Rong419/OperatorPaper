...
A molecular clock model is used to model how rates evolve along branches in the phylogenetic tree, so that a time tree can reconcile with the genetic distances between sequences. In the very beginning, a strict clock model assumes evolutionary rates to be the same at every branch \cite{zuckerkandl1965evolutionary}. However, a relaxed clock model allows rates to vary across lineages so that better estimates of divergence times can be obtained \cite{ho2005accuracy} \cite{lepage2007general} \cite{renner2005relaxed} \cite{knapp2005relaxed}. As an incomplete way of relaxing, a random local clock model only allows rates to be different within a subregion of the tree \cite{drummond2010bayesian}.  
%%define genetic distance
%%application of relaxed clock
...
%the application of UCR clock model 
An uncorrelated relaxed-clock analysis suggests an earlier origin for flowering plants\cite{smith2010uncorrelated}
...
This subsection further discusses how the ConstantDistance Operator makes a proposal and elucidate the correctness of the proposal in the aspect of a certain tree.  \cite{cooper2001complete}
%%introduce ratites data source 
