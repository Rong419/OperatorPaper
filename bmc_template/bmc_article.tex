...
\caption{\csentence{Efficiency comparison of continuous rates.}}
\label{ucldscalar}
\end{figure}
...
&Cons&12.3820&&164.52\\
\hline
\multirow{2}*{Simulated 500 sites}&Category&0.4644&\multirow{2}*{rate.coeff}&793.27\\
&Cons&0.4805&&3047.09\\
\hline
\multirow{2}*{Simulated 1000 sites}&Category&1.9270&\multirow{2}*{rate.coeff}&1599.42\\
&Cons&0.4805&&6255.03\\
\hline
\end{tabular}
...
&PopulationSize Scale Operator&-&-&-&-&0.0802&0.0267&0.0624&0.0267&-&-\\
  \hline
\multirow{2}*{Substitution model}&Kappa Scale Operator&0.0096&0.0096&0.0009&0.0009&0.0009&0.0009&0.0009&0.0009&0.0185&0.0185\\
...
In the comparison of ESS for the clock standard deviation (denoted by ucld.stdev), we specified a normal scale operator in "Category" configuration. In "Cons" configuration, the UcldstdevScaleOperator is used to sample the clock standard deviation of continuous rates. To avoid the concern that the difference between "Category" and "Cons" is a results of how rates are parameterised (i.e. discrete or continuous), we ran analysis where continuous rates are sampled without using the ConstantDistance operator (denoted by "NoCons" configuration). The weights of the operators in "NoCons" are consistent with those in "Category" which is detailed in Table \ref{weights}. To give an example, we ran the analysis using two data sets, RSV2 and HIV-1, where both the clock standard deviation (denoted by ucld.stdev) and clock mean (denoted by ucld.mean) are sampled. The efficiency comparison between "Cons" and "NoCons" is summarised in Figure \ref{ucldscalar}. The figure shows ESS per hour of ucld.stdev, ucld.mean and the statistics of branch rates (rate mean, rate variance and rate coefficient) in 20 independent MCMC chains. As can be seen, on average, ESS per hour of ucld.stdev and ucld.mean are almost the same in the two configurations. Note that the comparison between "Cons" and "Category" in Figure \ref{eff_comp} also shows the efficiency of the two parameters is about the same. This suggests that continuous rate parameterisation does not affect the efficiency of ucld.stdev. Moreover, "Cons" configuration achieves larger ESS per hour of the three rate statistics. It is therefore indicated that the proposed operators improve the performance by sampling continuous rates more efficiently. However, we also noticed that the rate parameterisation does have an effect on the mixing performance in MCMC chains. In the future, we will further investigate how to parameterise branch rates to get better performance when using the proposed operators.
