...
\emph{Step 3} Propose a new node time for \textbf{X} by ${t_X}' = {t_X} + a$, where $a$ follows a Uniform distribution with a symmetric window size $w$, i.e. $a \sim U[ - w, + w]$. Make sure that the proposed time is valid, i.e. $\max \{ {t_1},{t_2}\}  < {t_X}' < {t_P}$ holds. Otherwise, we reject the proposal.
...
\emph{Step 5} Return the Green ratio ${\alpha_{IN}}$ (Refer to 'Calculating the Green Ratio').
...
MCMC operators must use reversible proposal functions to satisfy the detailed balance (Refer to 'The Green ratio'). Therefore, all four of our operators involve a final step of calculating the Green ratio for the acceptance.
...
Likewise, the Green ratio for Simple Distance, Small Pulley and  Big Pulley can be obtained by Eq.(\ref{HR2}), Eq.(\ref{HR3}) and Eq.(\ref{HR4}) correspondingly. More details of the determinant of Jacobian matrix are explained in the Appendix. 
...
\end{equation}
where ${\mathbf{J}} = {\nabla h(x, x')}$ represents a vector differential matrix of deterministic function $h$. $r = \frac{{p}(x',x)}{{p}(x,x')}\left|{\mathbf{J}}\right|$ is defined as Green ratio, and ${\mathbf{J}}$ makes the proposal have a symmetric measure on each subspace in state $x$ and $x'$.
