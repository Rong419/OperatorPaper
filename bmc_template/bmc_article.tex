...
The efficiency is of great significance in phylogenetic analysis based on Bayesian MCMC. In this paper, we have discussed the methods of making a tree proposal in MCMC algorithms and presented a tree operator, Constant Distance operator, whose key mechanism is to randomly increase or decrease a node time and update the rates according to the constant product of rates and time, i.e. the genetic distances. On condition that the genetic distances are constant, the proposed operator has different strategies to work on internal nodes and the root in a phylogenetic tree.

Through the tests that sample from known prior distributions, it is proved that the Constant Distance operator is able to samples the trees correctly. Besides, the reliability of the proposed operator isn elucidated by a well-calibrated simulation study. Based on both simulated and real data sets, the proposed operator shows better effectiveness and is able to sample the rates and times more efficiently. The efficiency is improved by at least a 4-fold increase in ESS per hour. 

In the correlation analysis, it is demonstrated that the operator maintains correlations between rates and times. Take an internal node as an example. Assuming the proposed operator increases its current node time, to maintain the distances on the three branches linked to this node, the rate of the branch above this node increases with it. The two rates of the branches below the node decrease accordingly. If the rate goes up (down) along with the increase of node time, then they have a positive (negative) correlation. The correctness of a proposal is further verified by sampling a fixed unrooted tree.

The proposed operator has been developed as a new package for BEAST2. It is believed that the work in this paper will make some contributions to the research in phylogenetic analysis by providing more efficient estimations.
