...
Firstly, a method called \textit{Exchange (,)} is designed to propose a new tree topology. As is seen Fig.\ref{exchangemethod}, node $R$ represents the root of the tree $g$, $C$ and $N$ are the child nodes of the root, $S$ and $M$ denote the child nodes of $C$. After the method \textit{Exchange (\textbf{M},\textbf{N})} is called, the following operations will be performed to propose a new tree $g'$ by exchanging one child node of the root \textbf{N} and one child node of its sibling \textbf{M}.
\begin{itemize}
