...
What's more, according to the experimental results, the key idea of the presented operator that maintains the genetic distances shows a novel direction for more efficient proposals in Bayesian MCMC. For example, the operations on the internal nodes, in the current study, involve one random internal node, one node time and three branch rates. If two or more nodes are selected, then more associated rates and node times can be sampled in one proposal, which is possible to achieve even better efficiency. Except that, the current operations maintain the genetic distances of three associated branches, and the newly-proposed rates are only determined by the proposed node time. Another possible way is to make changes direclty on one branch rate so that the genetic distance is changed as well. To keep the genetic distances constant, the other branch rate is determined by the sum of genetic distances minus the changed distance of previous branch. It is therefore our future work to improve the presented operator by a more efficient proposal method.

Last but not least, the presented operator maintains genetic distances by assuming the genetic distances are accurate. But it is inevitable that there are some uncertainties which lead to the bad estimations. In this situation, the presented operator would alternatives to MCMC such as Variational Bayes.
