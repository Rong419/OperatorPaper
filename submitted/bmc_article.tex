...
\section*{Discussion}
According to the analysis above, the presented operator is valid and able to improve the efficiency of MCMC sampling process. But how often the operator is chosen to make a proposal has an influence on the overall performance of Bayesian phylogenetic analysis. In BEAST2 software, the frequency is determined by the weights of different operators. In the current study, the presented operator have equivalent weights when it works with other operators. Moreover, we assigned equal weights on operations of internal nodes and those on root, in order to keep a balance during the sampling process. How to assign weights to achieve better performance is not studied in this paper, and users may assign different weights in practical. Hence, an optimal method of assigning weights still needs further investigation. 

What's more, according to the experimental results, the key idea of the presented operator that maintains the genetic distances shows a novel direction for more efficient proposals in Bayesian MCMC. For example, the operations on the internal nodes, in the current study, involve one random internal node, one node time and three branch rates. If two or more nodes are selected, then more associated rates and node times can be sampled in one proposal, which is possible to achieve even better efficiency. Except that, the current operations maintain the genetic distances of three associated branches, and the newly-proposed rates are only determined by the proposed node time. Another possible way is to make changes directly on one branch rate so that the genetic distance is changed as well. To keep the genetic distances constant, the other branch rate is determined by the sum of genetic distances minus the changed distance of previous branch. It is therefore our future work to improve the presented operator by a more efficient proposal method.

Last but not least, the presented operator maintains genetic distances by assuming the genetic distances are accurate. But it is inevitable that there are some uncertainties which lead to the bad estimations. In this situation, the presented operator would incur worse estimations and much computational time. In the future, the authors will work on alternatives to MCMC that are able to provide further improvements. Recently, variational approaches have been used to approximate Bayesian inference \cite{beal2003variational}. These approaches make inference as an optimisation problem and take advantages of a set of tractable distributions, which decreases the computational cost by avoiding high-dimensional integrals in MCMC sampling schemes. Potential works in Ref.\cite{zhang2018variational} and Ref.\cite{dang2019stochastic} indicate the effectiveness and efficiency of applying variational approaches to phylogenetic inference.
